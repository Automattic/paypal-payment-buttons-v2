...
class PayPal_Tracks_Test extends TestCase {
	...
	/**
	 * enqueue_scripts must never throw when Tracking is unavailable.
	 */
	public function test_enqueue_scripts_is_silent_noop_when_tracking_unavailable() {
		if ( PayPal_Tracks::is_available() ) {
			$this->markTestSkipped( 'Jetpack Tracking is loaded — no-op path not exercised.' );
		}
		...
		$this->assertTrue( true, 'enqueue_scripts must not throw when Tracking is unavailable.' );
	}
	...
	public function test_get_inline_event_script_is_empty_when_unavailable() {
		...
		$this->assertSame(
			'',
			PayPal_Tracks::get_inline_event_script( 'jetpack_paypal_button_rendered' )
		);
