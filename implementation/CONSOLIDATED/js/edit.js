...
export default function PayPalPaymentButtonsEdit( { attributes, setAttributes } ) {
	...
	const clientIdWarning = useMemo( () => {
		...
		if ( clientId.length < 20 ) {
			return __( 'This looks too short for a Client ID. Make sure you copied the full value.', 'jetpack-paypal-payments' );
		}
		if ( ! /^A[A-Za-z0-9_-]+$/.test( clientId ) ) {
			return __( 'PayPal Client IDs usually start with "A". Double-check you copied the Client ID, not the app name.', 'jetpack-paypal-payments' );
		}
		return null;
	}, [ clientId ] );
	...
	const handleCreateButton = useCallback( () => {
		...
		apiFetch( {
			...
			.then( response => {
				...
				setSuccessMessage( __( 'PayPal button and payment link created successfully!', 'jetpack-paypal-payments' ) );
				setIsEditing( false );
				setTouchedFields( {} );
			} )
			.catch( err => {
				...
				<div className="jetpack-paypal-payment-buttons__connect">
					...
					{ wizardStep !== 'welcome' && wizardStep !== 'success' && (
						<div className="jetpack-paypal-wizard__step-indicator">
							<span className={ `jetpack-paypal-wizard__step ${ wizardStep === 'dashboard' || wizardStep === 'credentials' ? 'is-active' : '' }` }>
								{ __( '1', 'jetpack-paypal-payments' ) }
							</span>
							<span className="jetpack-paypal-wizard__step-line" />
							<span className={ `jetpack-paypal-wizard__step ${ wizardStep === 'credentials' ? 'is-active' : '' }` }>
					...
					{ wizardStep === 'dashboard' && (
						<div className="jetpack-paypal-wizard__dashboard">
							...
							<ol className="jetpack-paypal-wizard__instructions">
								<li>{ __( 'Click the button below to open the PayPal Developer Dashboard', 'jetpack-paypal-payments' ) }</li>
					...
					{ wizardStep === 'credentials' && (
						<div className="jetpack-paypal-wizard__credentials">
							...
							<p className="jetpack-paypal-wizard__subtitle">
								{ __( 'Copy these from your app in the PayPal Developer Dashboard:', 'jetpack-paypal-payments' ) }
							</p>
							...
							{ environment === 'sandbox' && (
								<Notice status="warning" isDismissible={ false }>
									{ __( 'Sandbox mode — buttons will use test credentials.', 'jetpack-paypal-payments' ) }
								</Notice>
							) }
							...
							/>
							{ clientIdWarning && (
								<p className="jetpack-paypal-payment-buttons__field-warning">
									{ clientIdWarning }
								</p>
							) }
							...
							<div className="jetpack-paypal-wizard__secret-field">
								<TextControl
									...
									help={ __( 'Click "Show" in PayPal to reveal it, then copy.', 'jetpack-paypal-payments' ) }
									autoComplete="off"
								/>
								<Button
									variant="tertiary"
			...
			<div className="jetpack-paypal-payment-buttons__create-form">
				...
				<div className="jetpack-paypal-payment-buttons__form-actions">
					...
					>
						...
						{ ! isCreating && ! hasButton && __( 'Create Button & Link', 'jetpack-paypal-payments' ) }
					</Button>
