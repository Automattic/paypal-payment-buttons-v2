...
**Steps to fix:**
1. Check that your server can reach `api-m.sandbox.paypal.com` (sandbox) or `api.paypal.com` (production)
...
3. Disconnect the current Sandbox connection
4. Reconnect using your **Production** (Live) credentials from the PayPal Developer Dashboard
5. Recreate your buttons — sandbox buttons don't work in production

### Credential Storage

- Credentials are encrypted with AES-256-CBC in `wp_options`
- Encryption key is derived from `LOGGED_IN_KEY` + `LOGGED_IN_SALT` in `wp-config.php`
- Changing these constants will invalidate stored credentials (you'll need to reconnect)
- OAuth tokens are cached as WordPress transients and auto-refresh 5 minutes before expiry
