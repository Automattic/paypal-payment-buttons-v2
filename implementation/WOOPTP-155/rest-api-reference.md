...
Create a payment resource (button/link) via the PayPal API.
...
| `paypal_api_unprocessable_entity` | 422 | Invalid amount/currency/business rule |
...
2. Plugin stores credentials encrypted (AES-256-CBC) in `wp_options`
3. Plugin exchanges credentials for an OAuth access token via `POST /v1/oauth2/token`
4. Token cached in WordPress transient (with 5-minute early refresh buffer)
...
**BN Code:** All API requests include `PayPal-Partner-Attribution-Id: WooNCPS_Ecom_Wordpress` header.
