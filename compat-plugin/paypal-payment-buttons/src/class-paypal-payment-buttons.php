...
class PayPal_Payment_Buttons {
	...
	public function register_paypal_block() {
		// Try flat standalone layout first, then jetpack_vendor layout.
		$dist_dir = PAYPAL_PAYMENT_BUTTONS_DIR . 'dist/paypal-payment-buttons';

		if ( ! is_dir( $dist_dir ) ) {
			// Fallback to Jetpack monorepo vendor layout.
